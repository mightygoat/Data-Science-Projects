...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    def getURLs(ns: Node): Seq[URL] = {
      for (n <- ns; a <- n \\ "a") yield new URL(u,  a \@ "href" )
    }
    ...
    for (s <- getURLContent(u); url = getLinks(s); u <- MonadOps.asFuture(url)) yield  u
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable,Seq[URL] ]]] = {
    ...
    /*val futSeqUrl = for (url <- us2) yield wget(url)

    val eitherConv = MonadOps.sequence(futSeqUrl)

    Future.sequence(eitherConv)*/

    val futSeqUrl = for (url <- us2) yield wget(url)

    Future.sequence(MonadOps.sequence(futSeqUrl))
