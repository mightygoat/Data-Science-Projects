...
object MonadOps {
  ...
  def flatten[X](xyf: Future[Try[X]])(implicit executor: ExecutionContext): Future[X] = {
    for(y <- xyf; yy <- asFuture(y) ) yield yy
  }
  ...
  def flatten[X](xsfs: Seq[Future[Seq[X]]])(implicit ec: ExecutionContext): Future[Seq[X]] = {
    ...
    val y =  for(xf <- xsfs ) yield xf

    val y1=Future.sequence(y)
  ...
  def sequence[X](xy: Try[X]): Either[Throwable, X] = xy.transform(s => Success(Right(s)), f => Success(Left(f))).get
  ...
  def sequence[X](xfs: Seq[Future[X]])(implicit executor: ExecutionContext): Seq[Future[Either[Throwable, X]]] = for(x <- xfs) yield sequence(x)
